...
for cutoff_radius in cutoff_radii:
    ...
    measurement_cases.append(y.repeat(num_samples, 1, 1, 1))
    posterior_samples.append(x_hat)

dinv.utils.plot(
    [x_plot] + measurement_cases,
    titles=["original"] + case_titles,
    suptitle="Measurements with progressively smaller Fourier low-pass cutoffs",
    figsize=(figsize * (len(cutoff_radii) + 1), figsize * num_samples),
    max_imgs=num_samples,
    rescale_mode="clip",
    save_fn="ve_progressive_lowpass_measurements.png",
    save_dir="ve_progressive_lowpass_measurements",
)

dinv.utils.plot(
    [x_plot] + posterior_samples,
    titles=["original"] + case_titles,
    suptitle=f"Posterior samples for progressively smaller Fourier low-pass cutoffs (alpha={posterior_alpha})",
    figsize=(figsize * (len(cutoff_radii) + 1), figsize * num_samples),
    max_imgs=num_samples,
    rescale_mode="clip",
    save_fn="ve_progressive_lowpass_posterior.png",
    save_dir="ve_progressive_lowpass_posterior_samples",
)

...
class RidgeRegularizer(nn.Module):
    r""" Implements the (weakly) convex ridge regularizer (W)CRR. 
    ...
    The model is given by :math:`\reg{x}=\sum_{c} \psi_c(W_c x)` for filters :math:`W_c` and potentials :math:`\psi_c`. 
    The filters :math:`W_c` are realized by a concatenation multiple convolution
    ...
    def __get_conv_lip(self):
        ...
        for filt in reversed(self.filters):
            impulse = torch.functional.F.conv_transpose2d(impulse, filt.weight, padding=filt.padding)
    ...
    def __conv_transpose(self, x):
        x = x / torch.sqrt(self.__get_conv_lip())
        for filt in reversed(self.filters):
            x = torch.functional.F.conv_transpose2d(x, filt.weight, padding=filt.padding)
        return x
